...
export class FFTAnalyzer {
  ...
  constructor(config: Partial<FFTAnalyzerConfig> = {}) {
    ...
    this.window = this.createWindow(this.config.windowType, this.config.fftSize);
   ...
   */
  private extractFeatures(spectrum: Float32Array, timestamp: number): SpectralFeatures {
  ...
  private calculateSpectralFlux(spectrum: Float32Array): number {
    ...
    let flux = 0;
    const minLength = Math.min(spectrum.length, this.state.previousSpectrum.length);
  ...
  private calculateFrequencyBands(spectrum: Float32Array): Float32Array {
    ...
    for (let band = 0; band < this.config.frequencyBands; band++) {
      ...
      const endBin = Math.min(Math.ceil(endFreq / this.frequencyResolution), spectrum.length);
  ...
  private smoothBands(bands: Float32Array): Float32Array {
    ...
    for (let i = 0; i < bands.length; i++) {
      smoothed[i] = alpha * bands[i] + (1 - alpha) * this.state.smoothedBands![i];
    }
   ...
   */
  private findPeakFrequency(spectrum: Float32Array): { peakFrequency: number; peakMagnitude: number } {
    let maxMagnitude = 0;
    let peakBin = 0;
