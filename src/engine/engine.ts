...
export class Engine {
  ...
  private fastTick(): void {
    ...
    try {
      ...
      // 2. Обработка аудио
      const audioMetrics = this.audioAnalyzer.processFrame(audioFrame, Date.now());
      ...
      if (processingTime > this.config.maxFastLoopTime) {
        this.logger.warn(`Fast loop exceeded time budget: ${processingTime.toFixed(2)}ms`);
      }
      
    } catch (error) {
      this.logger.error('Error in fast tick', { error });
  ...
  private collectPerformanceMetrics(): void {
    ...
    this.state.performance.memoryUsage = memoryUsage.heapUsed / memoryUsage.heapTotal;
